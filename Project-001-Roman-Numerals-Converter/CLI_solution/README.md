...
aws ec2 create-security-group \
    --group-name roman_numbers_sec_grp_okt \
    --description "This Sec Group is to allow ssh and http from anywhere"
```

- We can check the security Group with these commands
```bash
aws ec2 describe-security-groups --group-names roman_numbers_sec_grp_okt
```
...
```
```bash
aws ec2 run-instances --image-id $LATEST_AMI --count 1 --instance-type t2.micro --key-name okt-aws --security-groups roman_numbers_sec_grp_okt --tag-specifications 'ResourceType=instance,Tags=[{Key=Name,Value=roman_numbers}]' --user-data file:///home/ec2-user/userdata.sh
or
...
aws ec2 run-instances \
    ...
    --key-name okt-aws \
    --security-groups roman_numbers_sec_grp_okt \
